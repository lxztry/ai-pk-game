...
class RoundRobinTournamentWithReplay(TournamentWithReplay):
    """支持回放的循环赛"""
    ...
    def run(self, verbose: bool = False):
        ...
        for i in range(len(self.agents)):
            for j in range(i + 1, len(self.agents)):
                ...
                match_name = f"{agent1.name}_vs_{agent2.name}"
...
class EliminationTournamentWithReplay(TournamentWithReplay):
    """支持回放的淘汰赛"""
